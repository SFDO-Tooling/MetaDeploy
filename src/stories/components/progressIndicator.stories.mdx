...
If a user must be logged in to install a plan, **"Log in"** will be the first step.
If it's possible to install the `Plan` into a `Scratch Org`, **"Create Scratch Org"**
will be the first step. If both options are possible, both will be listed in the first step.

If pre-install validation is required, another step is added before **"Install."**

Active but incomplete steps are empty and marked by a colored border. `Completed`
steps are filled with checkmarks.
